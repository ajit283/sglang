...
setup_pip_toolchain() {
    ...
    # uv holds the editable sdist lock for the whole build, cargo included, and
    # every CUDA job builds the same path in the shared ~/.cache/uv, so they all
    # contend on one lock. A cold Rust build already exceeds uv's 300s default,
    # failing concurrent jobs on the same host. Keep this well under the
    # 20-minute install-step cap so uv reports the timeout itself instead of the
    # step being killed.
