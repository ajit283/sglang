...
class ServerArgs:
    ...
    def _handle_unified_memory_pool(self):
        if not self.enable_unified_memory:
            return
        if self.disaggregation_mode != "null":
            # PD ships the pool as whole page/slot envelopes of PHYSICAL ids (see
            # UnifiedMLATokenToKVPool.get_contiguous_buf_infos), which constrains
            # the transfer backend, PP, and the compaction mode.
